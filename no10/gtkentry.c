...
int main (int argc, char *argv[]) {
	...
	g_signal_connect (GTK_OBJECT (window), "destroy", GTK_SIGNAL_FUNC (exit), NULL);
	...
	g_signal_connect (GTK_OBJECT(passwd_entry), "clicked",
			        GTK_SIGNAL_FUNC(input_completed),
			        message_label);
	user_hbox = gtk_hbox_new ( TRUE, 5 );
	passwd_hbox = gtk_hbox_new ( TRUE, 5 );
